...
def mock_domain_services():
    with patch(
        'infrastructure.ui.components.word_cloud_component.build_corpus'
    ) as mock_build, patch(
        'infrastructure.ui.components.word_cloud_component.get_stopwords'
    ) as mock_stopwords, patch(
        'infrastructure.ui.components.word_cloud_component.WordCloud'
    ) as mock_wc_class:
...
def test_render_no_column(mock_streamlit):
    ...
    mock_streamlit.info.assert_called_with(
        "No se encontró la columna 'comentarios' para generar la nube.")
...
def test_render_empty_comments(mock_streamlit):
    ...
    mock_streamlit.info.assert_called_with(
        "No hay comentarios disponibles para generar la nube de palabras.")
