...
from infrastructure.ui.utils.text import (
    to_pdf_compatible as _to_pdf_compatible,
    normalize_newlines as _norm_newlines,
)
...
class _ReportePDF(FPDF):
    ...
    def header(self) -> None:  # pragma: no cover
        ...
        self.cell(
            0,
            10,
            _to_pdf_compatible(self.titulo),
            new_x="LMARGIN",
            new_y="NEXT",
            align="C",
        )
    ...
    def footer(self) -> None:  # pragma: no cover
        ...
        self.cell(
            0, 10, _to_pdf_compatible(f"Página {self.page_no()}"), align="C"
        )
...
def generate_pdf_export(
    ...
    try:
        ...
        _render_comments(
            ...
            if comentarios_preparados is not None
            else datos.head(_MAX_COMMENTS),
            shown_count=shown_count,
        )
        contenido = pdf.output(dest="S")
        if not contenido or (
            isinstance(contenido, (str, bytes, bytearray))
            and len(contenido) == 0
        ):
        ...
        pdf.cell(
            0,
            10,
            _to_pdf_compatible("Error al generar el PDF"),
            new_x="LMARGIN",
            new_y="NEXT",
        )
...
def _preparar_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "num_palabras" not in datos.columns:
        datos["num_palabras"] = (
            datos["comentarios"].astype(str).str.split().str.len()
        )
...
def _render_resumen(pdf: _ReportePDF, datos: pd.DataFrame) -> None:
    ...
    pdf.cell(
        0,
        8,
        _to_pdf_compatible(f"Total de comentarios analizados: {len(datos)}"),
        new_x="LMARGIN",
        new_y="NEXT",
    )
    pdf.cell(
    ...
    pdf.cell(
        0,
        8,
        _to_pdf_compatible("Resumen por categoría"),
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    pdf.cell(
        60,
        _LINE_HEIGHT + 2,
        _to_pdf_compatible("Clasificación"),
        border=1,
        fill=True,
    )
    pdf.cell(
        40,
        _LINE_HEIGHT + 2,
        _to_pdf_compatible("Comentarios"),
        border=1,
        fill=True,
        align="C",
    )
    pdf.cell(
        50,
        _LINE_HEIGHT + 2,
        _to_pdf_compatible("Longitud promedio"),
        border=1,
        fill=True,
        align="C",
    )
    ...
    for _, fila in resumen.iterrows():
        clasificacion = _to_pdf_compatible(
            str(fila.get("Clasificacion", "Sin clasificación"))
        )
        ...
        pdf.cell(
            40,
            _LINE_HEIGHT + 2,
            _to_pdf_compatible(str(num_comentarios)),
            border=1,
            align="C",
        )
        pdf.cell(
            50,
            _LINE_HEIGHT + 2,
            _to_pdf_compatible(f"{longitud_prom:.1f}"),
            border=1,
            align="C",
        )
        pdf.ln(_LINE_HEIGHT + 2)
...
def _crear_figuras(
    datos: pd.DataFrame, color_map: Dict[str, str]
    ...
    x_label = (
        "Número de palabras"
        if "num_palabras" in datos.columns
...
def _render_charts(
    pdf: _ReportePDF, figuras: Iterable[Tuple[str, "plotly.graph_objs.Figure"]]
) -> None:
    ...
    pdf.cell(
        0,
        8,
        _to_pdf_compatible("Visualizaciones"),
        new_x="LMARGIN",
        new_y="NEXT",
    )
    pdf.ln(1)
    for titulo, figura in figuras:
        try:
            imagen = BytesIO(
                pio.to_image(figura, format="png", scale=2, engine="kaleido")
            )
            imagen.seek(0)
            pdf.set_font("Helvetica", "I", 10)
            pdf.cell(
                0, 6, _to_pdf_compatible(titulo), new_x="LMARGIN", new_y="NEXT"
            )
...
def _draw_comments_header(
    pdf: _ReportePDF, col1_w: float, col2_w: float, col3_w: float
) -> None:
    ...
    pdf.cell(
        col2_w,
        _LINE_HEIGHT + 2,
        _to_pdf_compatible("Calificación"),
        border=1,
        fill=True,
        align="C",
    )
...
def _render_comments(
    pdf: _ReportePDF, datos: pd.DataFrame, shown_count: int | None = None
) -> None:
    ...
    pdf.cell(
        0,
        8,
        _to_pdf_compatible("Comentarios recientes"),
        new_x="LMARGIN",
        new_y="NEXT",
    )
    if shown_count is not None:
        ...
        pdf.cell(
            0,
            6,
            _to_pdf_compatible(f"Mostrando {shown_count} comentario(s)"),
            new_x="LMARGIN",
            new_y="NEXT",
        )
    ...
    if datos.empty:
        ...
        pdf.cell(
            0,
            8,
            _to_pdf_compatible("No hay comentarios para mostrar."),
            new_x="LMARGIN",
            new_y="NEXT",
        )
        pdf.ln(4)
    ...
    col3_w = (
        (pdf.w - pdf.l_margin - pdf.r_margin) - col1_w - col2_w
    )  # Comentario
    ...
    for idx, (_, fila) in enumerate(seleccion.iterrows()):
        ...
        raw_comment = (
            str(fila.get("comentarios", "(Sin comentario)")).strip()
            or "(Sin comentario)"
        )
        ...
        n_est_by_chars = _estimate_lines_by_chars(
            pdf, comentario, text_w, line_h
        )
        # Holgura adaptativa: solo si la última línea va muy llena
        if comment_lines:
            last_line = comment_lines[-1]
            last_ratio = (
                (pdf.get_string_width(last_line) / text_w) if text_w > 0 else 0
            )
        ...
        row_height = max(
            line_h + 2, text_h_total + 2 * padding_y + safety_margin
        )
        ...
        pdf.cell(
            col1_w,
            row_height,
            _to_pdf_compatible(clasificacion),
            border=1,
            fill=fill_row,
        )
        pdf.cell(
            col2_w,
            row_height,
            _to_pdf_compatible(calificacion),
            border=1,
            align="C",
            fill=fill_row,
        )
...
def _estimate_lines_by_chars(
    pdf: FPDF, text: str, width: float, line_h: float, extra_lines: int = 0
) -> int:
    ...
    sample = _to_pdf_compatible(
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzñáéíóúÑÁÉÍÓÚ 0123456789"
    )
...
def _normalizar_color_map(
    color_discrete_map: Dict[str, str], categorias: Iterable[str]
) -> Dict[str, str]:
    """
