...
Este módulo se encarga de generar reportes PDF con tablas, gráficos y comentarios,
delegando el cálculo de métricas a los servicios del dominio.
"""
...
def generate_pdf_export(df: pd.DataFrame, color_map: Dict[str, str] = None) -> bytes:
    """
    ...
    Args:
        ...
        color_map: Mapa de colores utilizado en la vista principal. Si es None, usa el del dominio.
    ...
    try:
        ...
        if figuras:
            ...
            pdf.cell(0, 8, "No se pudieron generar las visualizaciones.", new_x="LMARGIN", new_y="NEXT")
            pdf.ln(4)
        ...
        if not contenido or (isinstance(contenido, (str, bytes, bytearray)) and len(contenido) == 0):
        ...
        pdf.cell(0, 10, "Error al generar el PDF", new_x="LMARGIN", new_y="NEXT")
...
def _preparar_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "num_palabras" not in datos.columns:
        datos["num_palabras"] = datos["comentarios"].astype(str).str.split().str.len()
...
def _render_resumen(pdf: _ReportePDF, datos: pd.DataFrame) -> None:
    ...
    if resumen.empty:
        pdf.set_font("Helvetica", "I", 10)
        pdf.cell(0, 8, "No hay datos suficientes para generar el resumen.", new_x="LMARGIN", new_y="NEXT")
        pdf.ln(4)
        return
...
def _crear_figuras(
    ...
    x_label = "Número de palabras" if "num_palabras" in datos.columns else "Número de caracteres"
...
def _render_charts(
    ...
    for titulo, figura in figuras:
        try:
            ...
            pdf.set_font("Helvetica", "I", 10)
            pdf.cell(0, 6, f"{titulo} (no disponible)", new_x="LMARGIN", new_y="NEXT")
            pdf.ln(4)
...
def _normalizar_color_map(
    ...
    """
    Normaliza el mapa de colores para manejar diferentes variaciones de texto.
