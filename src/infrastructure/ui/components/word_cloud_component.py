...
class WordCloudComponent:
    ...
    def render(self, df: pd.DataFrame, max_words: int = 120):
        ...
        Args:
            df: DataFrame con los datos a visualizar (debe contener
            columna 'comentarios')
        ...
        if not comentarios:
            st.info(
                "No hay comentarios disponibles para generar la nube de"
                " palabras.")
            return
        ...
        if not corpus:
            st.info(
                "Los comentarios disponibles no contienen suficiente texto "
    ...
    def _validate_data(self, df: pd.DataFrame) -> bool:
        ...
        if "comentarios" not in df.columns:
            st.info(
                "No se encontró la columna 'comentarios' para generar la"
                " nube.")
            return False
        return True
    ...
    def _render_word_cloud(self, word_cloud: WordCloud):
        ...
        st.caption(
            "Las palabras con mayor tamaño aparecen con mayor frecuencia en "
