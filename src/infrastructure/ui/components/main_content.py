...
class MainContent:
    ...
    def _render_analysis_display(self):
        ...
        suggestions = self._controller.get_suggestions_component()
        suggestions.render(df_filtered)
        ...
        if analysis_name:
            try:
                notes_component = self._controller.get_notes_component()
                notes_component.render(analysis_name)
            except AttributeError:
                pass # Protección por si falta actualizar controller
    
    def _apply_monthly_date_filter(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Aplica un filtro de fechas por mes/año al DataFrame según la selección del usuario.
        
        El filtro se basa en una columna 'fecha' (datetime). Si no existe, el DataFrame
        ...
        if df['fecha'].isna().all():
            st.info(
                "No se pudo interpretar ninguna fecha válida en la columna 'fecha'. "
                "Se mostrará la información completa sin filtrar por mes/año."
            )
            return df
        ...
        # Valores por defecto para fecha_inicio/fecha_fin en controles personalizados
        ...
        if rango_rapido == "Último mes":
            ...
            with col1:
                fecha_inicio = st.date_input(
                    "Mes/Año inicio",
                    value=default_start,
                    help="Selecciona cualquier día del mes; solo se usará el mes y año."
                )
            with col2:
        ...
        start_month = pd.to_datetime(fecha_inicio).to_period('M').to_timestamp()
        end_month = pd.to_datetime(fecha_fin).to_period('M').to_timestamp()
        
        if start_month > end_month:
            st.warning("La fecha de inicio es posterior a la fecha de fin. Se mostrará el rango completo.")
