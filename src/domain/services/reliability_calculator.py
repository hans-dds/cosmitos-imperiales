"""
Servicio de dominio para calcular la fiabilidad de las predicciones de sentimiento.
...
def add_reliability_column(
    ...
    Si se proporcionan probabilidades, usa esas. Si no, y use_rating_fallback
    es True, usa la calificación como fallback.
    
    Args:
        df: DataFrame con los datos
        probabilities: Serie con probabilidades máximas (opcional)
        use_rating_fallback: Si True, usa calificación cuando no hay probabilidades
    ...
    if probabilities is not None and len(probabilities) == len(df):
        ...
        df['Fiabilidad'] = probabilities.apply(calculate_reliability_from_probability)
