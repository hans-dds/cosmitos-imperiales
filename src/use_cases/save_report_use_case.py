...
class SaveReportUseCase:
    ...
    def __init__(self, report_repository: IReportRepository, reports_base_dir: str = 'reports'):
        self._report_repository = report_repository
        self._reports_base_dir = reports_base_dir
        os.makedirs(self._reports_base_dir, exist_ok=True)

    def execute(self, analysis_name: str, df: pd.DataFrame, report_format: str = 'pdf', color_map=None) -> Tuple[bool, str, str]:
        """
        ...
        source_file_name, date_range, comments_count = self._compute_metadata(analysis_name, df)
    ...
    def _compute_metadata(self, analysis_name: str, df: pd.DataFrame) -> Tuple[str, str, int]:
        ...
        if 'fecha' in df.columns:
            try:
                ...
                if not fechas.empty:
                    date_range = f"{fechas.min().date()} - {fechas.max().date()}"
            except Exception:
                date_range = None
