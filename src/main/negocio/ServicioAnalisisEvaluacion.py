...
class ServicioAnalisisEvaluacion:
    ...
    def __init__(self, ruta_modelo: str):
        ...
        try:
            ...
            print(f"ERROR CRÍTICO: No se encontró el archivo del modelo en la ruta '{ruta_modelo}'.")
            self.modelo = None
        
        except Exception as e:

            print(f"ERROR CRÍTICO: Falló joblib.load() por una razón inesperada: {e}")
            import traceback
            traceback.print_exc()
            self.modelo = None
    ...
    def realizar_analisis_sentimientos(self, datos: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.modelo is None:
            print("ERROR: El modelo no está cargado (self.modelo es None). No se puede realizar la predicción.")
            return datos
